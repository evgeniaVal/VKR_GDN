...
class Main():
    ...
    def run(self):
        ...
        print(f"Время прогноза: {total_test_time:.2f} секунд")
    ...
    def get_score(self, test_result, val_result):
        ...
        #csv_path = self.get_save_path()[1]
        #T, N = np_test_result.shape[1:]
        #data = {}
        #for j in range(N):
        #    data[f'sensor{j}_pred'] = np_test_result[0, :, j]
        #    data[f'sensor{j}_true'] = np_test_result[1, :, j]
        #data['label'] =        np_test_result[2, :, 0]
              
        #pd.DataFrame(data).to_csv(csv_path, index=False)
        #print(f'Данные test-result сохранены в {csv_path}')
